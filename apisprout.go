...
import (
	...
	exmpl "github.com/pbarker/apisprout/example"
...
func getTypedExample(mt *openapi3.MediaType) (interface{}, error) {
	...
	if mt.Schema != nil {
		return exmpl.OpenAPIExample(exmpl.ModeResponse, mt.Schema.Value)
	}
		...
		for name, header := range headers {
			if header.Value != nil {
				...
				if header.Value.Schema != nil && header.Value.Schema.Value != nil {
					if v, err := exmpl.OpenAPIExample(exmpl.ModeResponse, header.Value.Schema.Value); err == nil {
